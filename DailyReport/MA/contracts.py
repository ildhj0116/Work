...
'SHF':['CU','ZN','AL','NI','AU','AG','SN','BU','RU','HC','RB'],
'CFE':['IC','IH','IF','T','TF'],
...
class contracts:
    ...
    def exchange_cmt(self,exchange='ALL'):
        return commodities[exchange]
    
    def exchange(self,cmt):
    ...
    def generate_cnt(self,month_=9,year_=17):
        dic={}
        for i in ['DCE','CZC','SHF','CFE']:
    ...
    def main_contracts(self,testday,top=2):
        '''
        select main contracts according to daily volume 
        top:if top=1, choose contract whose volume is largest ...
        '''
        year,month,day= testday.split('-')
        year,month,day = int(year[-2:]), int(month), int(day)
#        print(year,month,day)
#        return
        sets=self.generate_cnt(month,year)
        x={}
        result={}
        for key in sets:
            cntlist=sets[key]
#            print(cntlist)
        ...
        # x is the dictionary of dictionary  
        for key in x:
...
if __name__ == "__main__":
    dd=contracts()
    d= dd.main_contracts('2017-10-16')
    #kk=m1510('RB',10,17,'SHF')
    pprint.pprint(d)
